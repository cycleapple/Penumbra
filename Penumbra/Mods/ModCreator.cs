    ...
    GamePathParser gamePathParser) : IService
{
    ...
    {
        if (mod.FileVersion <= ModMeta.FileVersion)
        {
            mod.Groups.Clear();
            var changes = false;
        ...
        else
        {
            ...
            var groups = JObject.Parse(File.ReadAllText(metaFile))["Groups"] as JArray ?? [];
            foreach (var token in groups)
            {
                try
                {
                    var group = LoadModGroup(mod, token as JObject ?? new JObject());
                    if (group != null && mod.Groups.All(g => g.Name != group.Name))
                        mod.Groups.Add(group);
                }
                catch (Exception e)
                {
                    Penumbra.Log.Error($"Could not read V4 mod group for {mod.Identifier}:\n{e}");
                }
            }
        }
    }
    ...
    public void LoadDefaultOption(Mod mod)
    {
        ...
        else
        {
            ...
            try
            {
                var jObject = JObject.Parse(File.ReadAllText(metaFile));
                SubMod.LoadDataContainer(jObject["DefaultData"] as JObject ?? new JObject(), mod.Default, mod.ModPath);
            }
            catch (Exception e)
            {
                Penumbra.Log.Error($"Could not parse V4 default data for {mod.Name}:\n{e}");
            }
        }
    }
